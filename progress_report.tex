...
\begin{itemize}[noitemsep]
    ...
    \item Retrieval is performed over the 10 candidate passages provided per question
          in the distractor setting, rather than a full corpus. This inflates Recall@5
          scores. Full corpus retrieval using a FAISS index is planned for the next phase.
\end{itemize}
...
\paragraph{Retrieval-Augmented Generation --- Lewis et al., 2020.}
Lewis et al.\ proposed the RAG framework, which combines a dense retriever with
a sequence-to-sequence generator to produce answers grounded in retrieved passages.
This work directly motivates our pipeline architecture, where a retriever surfaces
relevant evidence and a reader generates the final answer.

\paragraph{ColBERT --- Khattab and Zaharia, 2020.}
ColBERT introduced late interaction retrieval, encoding query and passage tokens
independently but computing similarity via token-level interactions. It achieves
higher retrieval accuracy than standard bi-encoders at the cost of increased index
size, and serves as a reference point for understanding retrieval quality trade-offs
in our ablation design.
...
punctuation-removed tokenization. Evaluation results on 500 validation examples are
...
Evaluation results are reported alongside BM25 in Table~\ref{tab:results}.
...
\begin{tabular}{llll}
\toprule
\textbf{Model} & \textbf{Recall@5} & \textbf{F1} & \textbf{Exact Match} \\
\midrule
BM25                    & 0.972 & 0.060 & 0.004 \\
DPR (Vanilla)           & 0.982 & 0.064 & 0.002 \\
DPR + Hard Negatives    & —     & —     & —     \\
\bottomrule
\end{tabular}
...
gold passage in the top 5 results for 97.2\% of questions. Vanilla DPR achieves
0.982, a marginal improvement of 1.0 percentage points. This gap is expected to be
small: the DPR model used here was pretrained on Natural Questions, not HotpotQA,
so no domain adaptation has been applied. The improvement demonstrates that even
off-the-shelf dense retrieval outperforms lexical matching on this dataset; we
expect a larger gain once the encoders are fine-tuned with hard negatives on the
HotpotQA training split.

The low F1 (0.060) and Exact Match (0.004) scores reflect the absence of a reader
component: the predicted answer is approximated as the first sentence of the top
retrieved passage, which is rarely a short exact-match string. These scores will
improve once a reader model or LLM is integrated in the next phase.

F1 and EM scores are therefore not meaningful at this stage and are reported only
for completeness.

% ---------------------------------------------------------------
\section{Methodology and Implementation Progress}
...
\begin{enumerate}[noitemsep]
    \item \textbf{Hard Negative Generation (Completed):} BM25 was run on the training
          split to retrieve top-$k$ passages per question. Passages scoring highly under
          BM25 but not containing the gold answer were selected as hard negatives and
          saved to \texttt{data/hard\_negatives.json}. The initial run covers 1,000
          training examples as a proof-of-concept to validate the pipeline; full-scale
          mining over the complete 90k training split will be performed prior to
          fine-tuning on GPU.
...
\subsection*{Error Analysis Plan}
...
|   +-- hard_negative_mining.py # BM25-based hard negative generation
+-- results/
|   +-- bm25_results.json       # BM25 evaluation output
|   +-- dpr_results.json        # DPR evaluation output
+-- data/
|   +-- hard_negatives.json     # Mined hard negatives (1,000 train examples)
+-- requirements.txt
+-- .gitignore
+-- README.md
\end{lstlisting}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{DPR fine-tuning not yet started:} Hard negative mining output
          has been generated (\texttt{data/hard\_negatives.json}, 1{,}000 training
          examples). Fine-tuning the DPR encoders with these negatives is the focus
