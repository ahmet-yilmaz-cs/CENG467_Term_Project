...
\institute{
     300201066 \quad 300201067 \quad 290201066 \\
...
outperforms the larger Qwen2.5-7B-Instruct~\cite{qwen2.5} and the smaller
Phi-4-mini-Instruct~\cite{phi4}. Few-shot prompting with answer post-processing helps the
...
\noindent\textbf{Reader fine-tuning (separate experiment).}
To isolate the retrieval contribution, the main ablation deliberately uses a
fixed, zero-shot reader: it is a neutral instrument, identical across all
configurations, and it shows that the retrieval gains transfer to a
general-purpose reader without benchmark-specific training. As a separate
experiment, we LoRA fine-tune Flan-T5-xl on the HotpotQA \emph{train} split only
(8{,}000 examples, gold-passage context) and evaluate it on the same validation
questions with the DPR + HN retriever. Fine-tuning raises end-to-end
performance only modestly --- F1 $0.583 \to 0.613$, EM $0.466 \to 0.492$ ---
whereas the retrieval improvement contributed roughly $+0.10$ F1. This confirms
that the reader is not the bottleneck: retrieval is. We therefore report reader
fine-tuning here rather than folding it into the main ablation.
...
add a cross-encoder reranking stage to lift both-passage recall, and evaluate
over a full open-domain Wikipedia corpus for an even more realistic retrieval
scenario.
...
\bibitem{qwen2.5}
Qwen Team: Qwen2.5 Technical Report. arXiv preprint arXiv:2412.13663 (2024)

\bibitem{phi4}
Microsoft: Phi-4 Technical Report. arXiv preprint arXiv:2411.15124 (2024)
