...
and nDCG@10 (0.811 vs.\ 0.688). End-to-end evaluation with Flan-T5-xl achieves
F1\,=\,0.539 and Exact Match\,=\,0.428. An ablation study over three retrieval
configurations shows that the pretrained (Natural Questions) DPR only matches
the lexical BM25 baseline and that the improvement is driven entirely by hard
negative fine-tuning; qualitative error analysis identifies yes/no questions
and multi-hop reasoning as the dominant remaining failure modes.
...
negative mining and DPR fine-tuning occur offline; at inference time,
only the question encoder, FAISS index, and Flan-T5-xl reader are active.
...
% Labels
\node[lbl, above=1mm of hn,  xshift=18mm] {Offline (training)};
...
with BM25 and fine-tunes DPR to build a FAISS index. At inference,
the question encoder retrieves top-$k$ passages which Flan-T5-xl reads
...
We mine 30{,}000 examples from the HotpotQA train split, producing up to 5 hard
negatives each, and reserve 20\% as a held-out diagnostic set.
...
diagonal; all remaining in-batch positives together with all hard negatives
serve as negatives:
\begin{equation}
    \mathcal{L} = -\frac{1}{B} \sum_{i=1}^{B}
    \log \frac{\exp\!\left(f(q_i)^\top f(p^+_i)\right)}
         {\displaystyle\sum_{j} \exp\!\left(f(q_i)^\top f(p_j)\right)}
\end{equation}
where $f(\cdot)$ is the encoder's pooler output (768 dimensions). We use
AdamW with $lr = 2\times10^{-5}$, gradient clipping (max\_norm\,=\,1.0),
and a linear LR scheduler with 10\% warmup steps to stabilise contrastive
training. We apply a \textbf{curriculum strategy}: epoch~1 uses 1 hard
negative per question, epoch~2 uses 3, and epoch~3 uses the full 5. This
prevents early gradient instability caused by excessively difficult
negatives at the start of training. We checkpoint every epoch and select the
final model by \textbf{global-corpus retrieval} on the ablation set, not by
the per-question mini-pool used during training: with only the $\sim$7
in-batch candidates per question, the mini-pool metric saturates and
correlates poorly with open-corpus retrieval. The final model
(\textit{DPR + HN}) is trained on the full $\sim$24{,}000-example training
split with batch size 16.
...
All unique passages from the 500 validation examples (4{,}937 passages) are
encoded with the context encoder and indexed using FAISS
...
We compare three configurations under identical conditions (same 4{,}937-passage
global FAISS corpus, same Flan-T5-xl reader):
...
global FAISS corpus (4{,}937 passages). Recall@k is the fraction of questions
...
DPR + HN               & \textbf{0.539} & \textbf{0.428} \\
\bottomrule
\end{tabular}
\end{table}
...
every cutoff (R@3 +8.2, R@5 +7.4 over vanilla) and, crucially, at the top of
...
The pretrained DPR (Natural Questions) does \emph{not} outperform the lexical
BM25 baseline: BM25 is higher at R@3, R@5, and R@10 (e.g.\ 0.980 vs.\ 0.932 at
R@10) and essentially tied at MRR@10 and nDCG@10, while DPR leads only
marginally at R@1 (0.714 vs.\ 0.704). This is expected --- vanilla DPR has not
been adapted to HotpotQA's multi-hop bridging structure --- and shows that
out-of-the-box dense retrieval is not a free improvement over lexical search.
The benefit appears only after hard negative fine-tuning, which surpasses both
baselines decisively across all metrics.

\subsection{End-to-End QA Performance}

The full pipeline (DPR + HN + FAISS + Flan-T5-xl) achieves F1\,=\,0.539 and
EM\,=\,0.428 (Table~\ref{tab:qa}). The same Flan-T5-xl reader on the vanilla
DPR and BM25 retrievers reaches only F1\,=\,0.442 and F1\,=\,0.460
respectively, so the retrieval gains from hard negative mining translate
directly into a +0.08--0.10 F1 and +0.08 EM improvement downstream. The reader
runs zero-shot (no task-specific fine-tuning); its absolute scores are bounded
by this and by multi-hop questions that require synthesising both supporting
passages, analysed next.
...
\paragraph{Reader failures on yes/no questions.}
HotpotQA contains approximately 20\% yes/no questions. Flan-T5-xl
occasionally generates explanatory text rather than a binary answer:
\begin{quote}
\small
...
\end{quote}
The prediction is semantically correct but fails exact match. This failure
mode is orthogonal to retrieval quality: even perfect retrieval cannot
resolve it without a dedicated yes/no classification head or stronger
...
yielding partial F1 and EM\,=\,0. This reflects the difficulty of
multi-step inference when the required specificity is distributed across
two passages.
...
The jump from vanilla DPR (R@1 0.714, on par with BM25) to DPR + HN
(R@1 0.844) validates our core hypothesis: the gain comes from
...
the global 4{,}937-passage corpus instead is what reliably identifies the best
...
The Flan-T5-xl reader runs zero-shot. The dominant remaining failure modes
(yes/no questions, multi-hop inference, and context truncation) are reader-
and budget-side limitations rather than retrieval failures; a stronger or
fine-tuned reader and a larger context budget would partially address them.
...
to the end-to-end QA metrics (F1\,=\,0.539, EM\,=\,0.428). Our ablation shows
that out-of-the-box dense retrieval merely matches BM25 on this multi-hop
benchmark --- the improvement is attributable entirely to hard negative
fine-tuning --- and that selecting checkpoints on the global corpus rather
than a small per-question pool is essential for a faithful comparison. Future
work should scale hard negative mining to the full 90k training split,
...
Overwijk, A.: Approximate nearest neighbor negative contrastive estimation
